...
public class SASRenewal_RetryTests {
	...
	public Object[][] renewalRetryPostiveDataProvider() {
		return new Object[][] {};
		/*
		 * RENEWAL RETRY HAS BEEN REMOVED FROM FUNCTIONALITY { "RENEWAL_RETRY",
		 * "ACTIVATED", "SUCCESS", "CHARGING", "renewal" }, { "RENEWAL_RETRY",
		 * "ACTIVATED", "ERROR", "CHARGING", "renewal" }, { "RENEWAL_RETRY",
		 * "ACTIVATED", "FAILURE", "CHARGING", "renewal" }, { "RENEWAL_RETRY",
		 * "SUSPEND", "ERROR", "CHARGING", "renewal" }, { "RENEWAL_RETRY", "SUSPEND",
		 * "FAILURE", "CHARGING", "renewal" }, { "RENEWAL_RETRY", "SUSPEND",
		 * "LOW_BALANCE", "CHARGING", "renewal" }, { "RENEWAL_RETRY", "SUSPEND",
		 * "IN_PROGRESS", "CHARGING", "renewal" }, { "RENEWAL_RETRY", "SUSPEND",
		 * "NOTIFICATION_WAIT", "CHARGING", "renewal" },
		 */
	}

	@Test(dataProvider = "renewalRetryPostiveDataProvider", groups = { "scrap" })
	public void renewalRetryPostiveTests(String activityType, String currentSubscriptionState, String transactionState,
		...
		try {
			...
			Message message = RabbitUtil.receive(RabbitMQConnection.getRabbitTemplate(),
					RabbitUtil.getQueueName(productId, partnerId, countryCode, actionTable), 10000);
